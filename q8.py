...
def settings_screen_v2():
    ...
    while True:
        ...
        if draw_button("Тема", WINDOW_WIDTH // 2 - 100, y_position, 200, 40, (220, 220, 220), (200, 200, 200), events):
            theme_settings_screen()
        y_position += 60
        if draw_button("Рекорд", WINDOW_WIDTH // 2 - 100, y_position, 200, 40, (220, 220, 220), (200, 200, 200),
                       events):
            record_settings_screen()
        y_position += 60
        if draw_button("Назад", WINDOW_WIDTH // 2 - 50, WINDOW_HEIGHT - 100, 100, 40, (220, 220, 220), (200, 200, 200),
                       events):
            return
        pygame.display.flip()
        clock.tick(60)
    ...
    while True:
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                sys.exit()
        screen.fill(THEMES[current_theme]["background"])
        title_surface = font.render("Выбор темы", True, get_text_color())
        title_rect = title_surface.get_rect(center=(WINDOW_WIDTH / 2, WINDOW_HEIGHT / 6))
        screen.blit(title_surface, title_rect)

        y_position = WINDOW_HEIGHT // 4
        for theme_name in THEMES:
            if draw_button(theme_name, WINDOW_WIDTH // 2 - 100, y_position, 200, 40, (220, 220, 220), (200, 200, 200),
                           events):
...
def record_settings_screen():
    global high_score, db_conn
    while True:
        events = pygame.event.get()
        for event in events:
            if event.type == pygame.QUIT:
        ...
        screen.fill(THEMES[current_theme]["background"])
        title_surface = font.render("Рекорд", True, get_text_color())
        title_rect = title_surface.get_rect(center=(WINDOW_WIDTH / 2, WINDOW_HEIGHT / 6))
        screen.blit(title_surface, title_rect)

        record_text = font.render(f"Рекорд: {high_score}", True, get_text_color())
        record_rect = record_text.get_rect(center=(WINDOW_WIDTH / 2, WINDOW_HEIGHT // 2))
        screen.blit(record_text, record_rect)

        if draw_button("Сбросить", WINDOW_WIDTH // 2 - 100, WINDOW_HEIGHT // 2 + 40, 200, 40, (220, 220, 220),
                       (200, 200, 200), events):
            high_score = 0
...
def main_menu():
    while True:
        ...
        screen.blit(title_surface, title_rect)
        if draw_button("Начать", WINDOW_WIDTH / 2 - 75, WINDOW_HEIGHT / 2 - 50, 150, 50, (240, 240, 240),
                       (200, 200, 200), events):
            return "start"
        if draw_button("Настройки", WINDOW_WIDTH / 2 - 75, WINDOW_HEIGHT / 2 + 20, 150, 50, (240, 240, 240),
                       (200, 200, 200), events):
            return "settings"
        if draw_button("Выйти", WINDOW_WIDTH / 2 - 75, WINDOW_HEIGHT / 2 + 90, 150, 50, (240, 240, 240),
                       (200, 200, 200), events):
            return "exit"
        pygame.display.flip()
        clock.tick(60)
...
def game_over_screen():
    while True:
        ...
        title_rect = title_surface.get_rect(center=(WINDOW_WIDTH / 2, WINDOW_HEIGHT / 4))
        screen.blit(title_surface, title_rect)
        if draw_button("Начать заново", WINDOW_WIDTH // 2 - 100, WINDOW_HEIGHT // 2 - 80, 200, 50, (240, 240, 240),
                       (200, 200, 200), events):
            return "restart"
        if draw_button("Меню", WINDOW_WIDTH // 2 - 100, WINDOW_HEIGHT // 2 - 20, 200, 50, (240, 240, 240),
                       (200, 200, 200), events):
            return "menu"
        if draw_button("Выйти", WINDOW_WIDTH // 2 - 100, WINDOW_HEIGHT // 2 + 40, 200, 50, (240, 240, 240),
                       (200, 200, 200), events):
            return "exit"
        pygame.display.flip()
        clock.tick(60)
